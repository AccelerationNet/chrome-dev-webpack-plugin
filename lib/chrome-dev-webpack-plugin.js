...
module.exports = class ChromeDevWebpackPlugin {
  ...
  mapFilesToBundles (compilation, files) {
    ...
    files = (files || []).map(file => {
      return { file:file, fullPath: path.join(this.context, file)};
    });

    var getParentsV3 = (module) => {
      var parents = [];
      if (module.parents && 0 < module.parents.length) {
        for (var i = 0, len = module.parents.length; i < len; ++i) {
    ...
    var getEntryPathV4 = (chunks) => {
      let entryPath;
      chunks && chunks.forEach(chunk => {
        const entry = chunk.entryModule;
        if (entry) {
          const request = entry.userRequest;
          if (request) {
            entryPath = request;
            return;
          }
    ...
    var addToBundleMap = (entry, expansion) => {
      for (var i = 0, len = files.length; i < len; ++i) {
        var file = files[i];
        if (file.fullPath === entry || file.file === entry) {
          filesBundlesMap[file.file] = expansion;
        }
      }
    };

    // The chunking was refactored significantly between webpack 3 and 4.
    // Detect if we have the webpack4 schema, else it must be 1-3
    if (compilation.chunkGroups) {
    // Webpack 4 explore each chunkGroup
      compilation.chunkGroups.forEach(group => {
        this.log("mapping ", group.files);
        const scriptSet = group.getFiles();
        if (!scriptSet || 0 === scriptSet.length) {
          return;
        }
        const entryPath = getEntryPathV4(group.chunks);
        if (!entryPath || 0 === entryPath.length) {
          return;
        }
        addToBundleMap(entryPath, scriptSet);
      });
    } else {
      // Webpack 1-3 Explore each chunk
        ...
        chunk.modules.forEach(module => {
          if(module.fileDependencies) {
            ...
            module.fileDependencies.forEach(filepath => {
              ...
              if (filepath) {
                let parents = getParentsV3(chunk);
                addToBundleMap(filepath, parents.concat(chunk.files));
              }
            });
          }
        });
